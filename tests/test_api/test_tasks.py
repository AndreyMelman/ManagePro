...
async def test_create_task(client, create_user, get_token):
    ...
    team_resp = await client.post(
        "/api/v1/teams", json={"name": "TaskTeam", "code": "TASKT"}, headers=admin_headers
    )
...
async def test_update_task_not_owner(client, create_user, get_token):
    ...
    team_resp = await client.post(
        "/api/v1/teams", json={"name": "OwnerTeam", "code": "OWNR"}, headers=owner_headers
    )
    task_resp = await client.post(
        "/api/v1/tasks",
    ...
    user_id = user["id"]
    await client.post(f"/api/v1/teams/{team_resp.json()['id']}/user/{user_id}", headers=owner_headers)
...
async def test_delete_task(client, create_user, get_token):
    ...
    admin_headers = await get_token(client, admin_email, admin_password)
    team_resp = await client.post(
        "/api/v1/teams", json={"name": "DelTaskTeam", "code": "DTASK"}, headers=admin_headers
    )
    task_resp = await client.post(
        "/api/v1/tasks",
...
async def test_delete_task_not_owner(client, create_user, get_token):
    ...
    owner_headers = await get_token(client, owner_email, owner_password)
    team_resp = await client.post(
        "/api/v1/teams", json={"name": "DelOwnerTeam", "code": "DLOWN"}, headers=owner_headers
    )
    task_resp = await client.post(
        "/api/v1/tasks",
